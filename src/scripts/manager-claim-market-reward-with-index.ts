...
import { reserveAddress, marketRewardIndex } from "../../config/kamino";
import {
  ADAPTOR_PROGRAM_ID,
  DISCRIMINATOR,
  FARM_GLOBAL_CONFIG,
...
const claimMarketRewardWithIndex = async (
  ...
  for (const [_, farmData] of farmsForUserArray) {
    ...
    console.log("Claimed market reward with index", rewardIndex, "signature:", txSig);
  }
};
...
const main = async () => {
  ...
  await claimMarketRewardWithIndex(
    ...
    DISCRIMINATOR.CLAIM_MARKET_REWARD_WITH_INDEX,
    marketRewardIndex,
    [lookupTableAddress]
  );
};
